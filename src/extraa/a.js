...
const BarcodeGenerator = () => {
  ...
  useEffect(() => {
    barcodes.forEach((barcode, index) => {
      const canvas = document.getElementById(`barcode_${index}`);
      JsBarcode(canvas, barcode, {
        format: "CODE128", // Replace with the desired barcode format
        displayValue: true, // Display the barcode value
        text: barcode + " " + "Mrp:Rs 400", // Replace with the desired barcode value
  ...
  const generateBarcodePDF = () => {
    ...
    for (let i = 0; i < barcodes.length; i++) {
      // const canvas = document.getElementById(`barcode_${barcodes[i]}`);
      // JsBarcode(canvas, barcodes[i], {
      //   format: "CODE128", // Replace with the desired barcode format
      //   displayValue: true, // Display the barcode value
      //   text: barcodes[i] + " " + "Mrp:Rs 400", // Replace with the desired barcode value
      //   width: 1.5, // 2 inches width
      //   height:40, // 1 inch height
      //   margin: 0,
      // });
      ...
      doc.text(barcodeValue, currentX, currentY + imageHeight + 5); // Display barcode value below the barcode image
  ...
  return (
    <div className="m-10">
      ...
      <div
        ref={containerRef}
        className="mt-5  grid grid-cols-3 p-4 gap-2 bg-red-400"
        style={{
          width: "8.27in",
          height: "",
        }}
      >
        {barcodes.map((barcode, index) => (
          <div
            key={index}
            className="border border-gray-600 flex justify-center p-2"
          >
